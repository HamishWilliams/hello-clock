// Format the fitbit count for display on a clock.
export function formatCount(i) {
  if (i != i.toFixed(0)) {
	// The value passed is not an integer (probably distance).  Drop excess digits.  
    i = i.toFixed(2);
  }	
  if (i >= 100000) {
	// The value passed is big!  Truncate it by returning it as a number of thousands (k).  
    i = (i / 1000).toFixed(2);
    i = i + "k";
  }
  i = i.toLocaleString();
...
// Return the give number as fixed-width digits.  Code provided by Fitbit.
